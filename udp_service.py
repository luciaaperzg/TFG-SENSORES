...
class UdpService:
    ...
    def start(self):
        ...
        self.socket.bind(("0.0.0.0", self.port))  # Cambiado a 0.0.0.0 para escuchar en todas las interfaces
        self.isOpen = True
        print('UDP服务器已启动...')
    ...
    def onReceive(self):
        ...
        while self.isOpen:
            try:
                ...
                print(f"Datos recibidos desde {address}: {data}")  # Depuración
                for var in data:
                    ...
                    if len(self.tempBuffer) == 12:
                        ...
                        print(f"ID detectado: {device_Id}")  # Depuración
                ...
                if not self.isOpen:  # Si el servidor se cerró, salir
                    break
